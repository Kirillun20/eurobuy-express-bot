...
import {
  CURRENCIES, DELIVERY_METHODS, PAYMENT_METHODS,
...
import {
  ...
  RefreshCw, Info, Coins, User as UserIcon, Building2, Eye, EyeOff, ShoppingCart, Globe,
} from 'lucide-react';
...
const emptyItem = () => ({
  link: '', name: '', quantity: '1', weight: '', price: '', currency: 'RUB', notes: '',
});
...
const AvitoOrderPage = () => {
  ...
  const serviceCostBYN = (() => {
    const percentCost = priceBYN * 0.15;
  ...
  const canAddItem = () => {
    return (currentItem.link || currentItem.name) && currentItem.price && !overWeight;
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (!profile) {
        profile = await createProfile({ name: regForm.name.trim(), email: regForm.email.trim(), phone: regForm.phone.trim() });
      }
      if (profile) {
  ...
  const submit = () => {
    ...
    if (!currentUser) { setStep('register'); return; }
  ...
  const buildPaymentDetails = (): PaymentDetails | undefined => {
    ...
    if (paymentMethod === 'card') return { cardNumber: cardNumber || 'Тестовая оплата', amount: grandTotal };
  ...
  const finalizeOrder = async (orderUser: User) => {
    ...
    try {
      ...
      const order: Order = {
        ...
        totalServiceBYN: roundBYN(totalServiceBYN), deliveryMethod, deliveryCostBYN,
        paymentMethod, status: 'pending', createdAt: new Date().toISOString(),
      ...
      if (created) {
        if (pointsEarned > 0) {
          ...
          const u = getUser();
          if (u) { u.euroPoints = (u.euroPoints || 0) + pointsEarned; saveUser(u); }
        }
          ...
          {progressSteps.map((s, i) => (
            <div key={s} className="flex items-center">
              <div className="flex flex-col items-center">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-semibold transition-all duration-500 ${
                  i <= progressIdx ? 'bg-gradient-to-br from-emerald-500 to-teal-500 text-primary-foreground shadow-lg shadow-emerald-500/30' : 'glass text-muted-foreground'
                }`}>
              ...
              {i < progressSteps.length - 1 && (
                <div className={`w-6 h-0.5 mx-1 mt-[-14px] rounded-full transition-colors duration-500 ${i < progressIdx ? 'bg-emerald-500' : 'bg-border'}`} />
              )}
            </div>
          ))}
        </motion.div>
                  ...
                  <motion.button key={platform.id} whileTap={{ scale: 0.98 }}
                    ...
                    className={`w-full flex items-center gap-3 p-4 rounded-xl transition-all duration-300 ${
                      selectedPlatform === platform.id
                        ? 'glass border border-emerald-500/40 shadow-lg shadow-emerald-500/10'
            ...
            <div className="space-y-4">
              ...
              {paymentMethod === 'card' && (
                <div className="glass rounded-2xl p-5 border-glow">
                  <div className="flex items-center gap-2 mb-3"><CreditCard size={18} className="text-emerald-500" /><h3 className="font-display font-bold text-sm">Тестовый режим</h3></div>
                  <p className="text-xs text-muted-foreground mb-4">Оплата картой в тестовом режиме.</p>
                  <div><Label className="text-xs mb-1.5 block text-muted-foreground">Номер карты</Label><Input placeholder="4242 4242 4242 4242" value={cardNumber} onChange={e => setCardNumber(e.target.value)} className={inputClass} /></div>
