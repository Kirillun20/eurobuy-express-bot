...
const ReviewsPage = () => {
  ...
  const handleSubmit = async () => {
    if (!name.trim() || !text.trim()) { toast.error('Заполните имя и текст отзыва'); return; }
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(-1)} className="w-9 h-9 rounded-xl glass border-glow flex items-center justify-center"><ArrowLeft size={16} className="text-muted-foreground" /></button>
            <div><h1 className="text-2xl font-display font-bold">Все отзывы</h1><p className="text-xs text-muted-foreground">{reviews.length} отзывов</p></div>
          </div>
        ...
        {loading ? <div className="text-center py-12 text-muted-foreground">Загрузка...</div> : (
          <motion.div variants={{ hidden: {}, show: { transition: { staggerChildren: 0.06 } } }} initial="hidden" animate="show" className="space-y-3">
            {reviews.map(r => (
              <motion.div key={r.id} variants={{ hidden: { opacity: 0, y: 15 }, show: { opacity: 1, y: 0, transition: { duration: 0.4 } } }}
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <div className="flex gap-0.5 mt-0.5">{Array.from({ length: 5 }, (_, i) => <Star key={i} size={10} className={i < r.rating ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground/20'} />)}</div>
                    </div>
                  </div>
